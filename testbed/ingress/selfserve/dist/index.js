...
exports.selfServeIngress = exports.SelfServeIngress = void 0;
...
class SelfServeIngress {
    constructor() {
        ...
        const secretKey = process.env.PF_SIGNATURE_SECRET ||
            "default-secret-key-change-in-production";
        this.validationMiddleware = new validation_1.ValidationMiddleware(secretKey);
        this.tenantRateLimiter = new validation_1.TenantRateLimiter(15 * 60 * 1000, 100); // 15 min window, 100 requests
    ...
    setupMiddleware() {
        ...
        this.app.use(validation_1.requestIdMiddleware);
    ...
    setupRoutes() {
        ...
        this.app.post("/api/validate-request", this.validationMiddleware.validateRequest, this.tenantRateLimiter.limitByTenant, this.handleValidateRequest.bind(this));
    ...
    // Tenant update
    async handleUpdateTenant(req, res) {
        try {
            const { tenantId } = req.params;
    ...
    // Middleware download
    async handleDownloadMiddleware(req, res) {
        try {
    ...
    // Postman collection
    async handleGetPostmanCollection(req, res) {
        const collection = {
            info: {
    ...
    // PF-Sig validation
    async handleValidatePfSig(req, res) {
        try {
    ...
    // Receipt validation
    async handleValidateReceipt(req, res) {
        try {
    ...
    setupTenantRateLimiting(tenantId, plan) {
    ...
    isValidPfSigFormat(signature) {
        // Basic format validation
        return /^pf_[a-f0-9]{64}$/.test(signature);
    }
    async verifyPfSig(signature, data) {
    ...
    validateReceiptSchema(receipt) {
        const errors = [];
        if (!receipt.id)
            errors.push("Missing receipt ID");
        if (!receipt.tenant)
    ...
    async handleValidateRequest(req, res) {
        try {
    ...
    async handleGenerateReceipt(req, res) {
        try {
...
exports.SelfServeIngress = SelfServeIngress;
// Validation schemas
...
const TenantUpdateSchema = zod_1.z.object({
    ...
    plan: zod_1.z.enum(["sandbox", "starter", "professional"]).optional(),
});
// Export the class and create instance
exports.selfServeIngress = new SelfServeIngress();
