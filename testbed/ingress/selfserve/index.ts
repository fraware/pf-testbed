...
export class SelfServeIngress {
  ...
  // Middleware download
  private async handleDownloadMiddleware(req: express.Request, res: express.Response): Promise<void> {
    try {
  ...
  // Postman collection
  private async handleGetPostmanCollection(req: express.Request, res: express.Response): Promise<void> {
    const collection = {
      info: {
          ...
          item: [
            {
              ...
              request: {
                ...
                body: {
                  mode: 'raw',
                  raw: JSON.stringify({ tenant_id: '{{tenant_id}}', name: 'My API Key' })
                }
              }
            }
          ]
  ...
  private getPlanLimits(plan: string): { windowMs: number; maxRequests: number } {
    switch (plan) {
      case 'sandbox':
  ...
  private async sendWelcomeEmail(tenant: Tenant, apiKey: ApiKey): Promise<void> {
  ...
  private validateReceiptSchema(receipt: any): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
