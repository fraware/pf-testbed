...
class ABACEngine {
    ...
    async evaluateAccess(request) {
        ...
        const hasValidRole = request.subject_roles.some(role => tenantContext.allowed_roles.includes(role));
        if (!hasValidRole) {
    ...
    generateMockResults(request, tenantContext) {
        const results = [];
